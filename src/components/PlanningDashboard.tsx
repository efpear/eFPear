    ...
    <tr className={`border-b border-slate-100 ${mod.excluido ? "bg-orange-50/40" : "bg-white"}`}>
      ...
      <td className="px-3 py-2">
        <input
          ...
          value={mod.horas}
          onChange={(e) =>
            onUpdate(mod.id, { horas: parseInt(e.target.value) || 0 })
          }
        />
      </td>
      <td className="px-3 py-2">
...
function ImparticionDashboard({
  ...
  // Simple sequential distribution: each module follows the previous
  ...
  for (const mod of lectivos) {
    const semanasNeed =
      horasSemanales > 0 ? Math.ceil(mod.horas / horasSemanales) : 0;
  ...
  const fechaFinGlobal =
    entries.length > 0 ? entries[entries.length - 1].fin : fechaInicio;
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <div className="bg-slate-50 border border-slate-200 rounded-lg p-3 text-center">
          <div className="text-xl font-bold text-slate-700">{fechaFinGlobal}</div>
          ...
          {excluidos.length > 0 && (
            <div className="px-4 py-3 bg-orange-50/50">
              <div className="text-[10px] text-orange-600 font-medium mb-1">
                Modulos de practicas (no computan para fechas):
              </div>
              {excluidos.map((m) => (
                <div
                  key={m.id}
                  className="flex items-center gap-2 text-[10px] text-orange-700 py-0.5"
                >
                  <span className="font-mono">{m.codigo}</span>
                  <span className="text-orange-400">&mdash;</span>
                  <span>{m.titulo}</span>
...
// ============================================================
// MAIN COMPONENT: PlanningDashboard (Block A)
// ============================================================
...
let _nextId = 1;
function newId() {
  return `manual-${_nextId++}`;
}

export function PlanningDashboard({ fechaInicio }: PlanningDashboardProps) {
  ...
  return (
    <div className="space-y-4">
      {/* ENTRY PANEL — upload or manual */}
      {!dataset && (
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6 space-y-4">
          <div>
            <h3 className="text-sm font-semibold text-slate-900">
              Planificacion de fechas de imparticion
            </h3>
            <p className="text-xs text-slate-500 mt-1">
              Sube una ficha SEPE, certificado o anexo en PDF — o introduce los modulos manualmente.
            </p>
          </div>
            ...
            {isLoading ? (
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-slate-400">
                  Ficha SEPE · Certificado completo · Anexo del certificado
                </p>
              </div>
            )}
          </div>
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 bg-slate-50 flex items-center justify-between">
            <div>
              ...
              {dataset.source === "pdf" && (
                <p className="text-[10px] text-slate-500 mt-0.5">
                  Extraido del PDF · Revisa y ajusta si es necesario
                </p>
              )}
            </div>
          ...
          {dataset.warnings.length > 0 && (
            <div className="px-4 py-2 bg-amber-50 border-b border-amber-100">
              <p className="text-[10px] text-amber-700">
                ⚠ {dataset.warnings.slice(0, 2).join(" · ")}
              </p>
            </div>
          )}
                ...
                <tr className="border-t border-slate-200 bg-slate-50">
                  ...
                  <td className="px-3 py-2 text-right">
                    <div className="text-xs font-semibold text-green-700">
                      {dataset.totalHorasLectivas}h lectivas
                    </div>
                    <div className="text-[10px] text-orange-600">
                      +{dataset.totalHorasPracticas}h practicas
                    </div>
                  </td>
                  <td colSpan={3} />
                </tr>
      ...
      {/* DASHBOARD — imparticion calendar */}
